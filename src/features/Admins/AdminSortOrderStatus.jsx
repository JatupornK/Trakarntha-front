...
import { orderStatus, sortOrder } from "../../stores/adminSlice";
import { useDispatch, useSelector } from "react-redux";
import { AiOutlineDown, AiOutlineUp } from "react-icons/ai";

export default function AdminSortOrderStatus() {
  const [isSort, setIsSort] = useState(false);
  const ref = useRef();
  const { sortOrderStatus } = useSelector((state) => state.admin);
  ...
  const handleClickSort = (item) => {
    dispatch(sortOrder({type:'order',sort: item}));
    setIsSort(false);
  };
  ...
  return (
    <>
      <div className="text-center pt-4 pb-2" ref={ref}>
        ...
        >
          <p className="flex-1">
            {sortOrderStatus === "All" ? "Order status" : sortOrderStatus}
          </p>
                ...
                {orderStatus.map((item, idx) => (
                  <li
                    ...
                    className={`hover:bg-gray-100 p-1 leading-5 ${
                      sortOrderStatus === item && "bg-gray-200"
                    }`}
                    key={idx}
                  >
                    {item}
